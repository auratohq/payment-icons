...
(async () => {
  ...
  assert.match(await page.locator('#stats').innerText(), /2,274/);
  assert.match(await page.locator('#stats').innerText(), /6,010/);
  await visibleImagesReady(page);
  await page.screenshot({ path: path.join(output, 'desktop.png') });
  await page.locator('#load-more').click();
  assert.equal(await page.locator('.tile').count(), 120);
  await page.locator('#search').fill('pmt_in3');
  assert.equal(await page.locator('.tile').count(), 1);
  await page.locator('.tile').click();
  assert.equal(await page.locator('#detail-path').inputValue(), 'payment-methods/pmt_in3.png');
  await page.waitForFunction(() => document.getElementById('detail-image').naturalWidth === 256);
  await page.locator('#copy-path').click();
  assert.equal(await page.evaluate(() => navigator.clipboard.readText()), 'payment-methods/pmt_in3.png');
  const downloadEvent = page.waitForEvent('download');
  await page.locator('#download-icon').click();
  const download = await downloadEvent;
  assert.equal(download.suggestedFilename(), 'pmt_in3.png');
  ...
  assert.match(await page.locator('#result-count').innerText(), /^6,004 assets/);
  await page.locator('#source-filter').selectOption('all');
  assert.match(await page.locator('#result-count').innerText(), /^6,010 assets/);
